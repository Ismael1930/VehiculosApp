...
function registrarCli(e) {
    e.preventDefault();
    const dni = document.getElementById("cedula").value;
    const nombre = document.getElementById("nombre").value;
    const telefono = document.getElementById("apellido").value;
    const direccion = document.getElementById("fechaNacimiento").value;
    if (dni == '' || nombre == '' || telefono == '' || direccion == '') {
        alertas('Todo los campos son obligatorios', 'warning');
    } else {
         const url = '../../Controllers/ClienteController/agregarCliente';
         const frm = document.getElementById("formulario");
         const http = new XMLHttpRequest();
         http.open("POST", url, true);
         http.send(new FormData(frm));
